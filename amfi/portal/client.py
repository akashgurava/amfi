...
class AmfiClient:
    ...
    async def _fetch_data_rows(
        ...
        *,
        ...
        if data_rows is None and payload.get("message") == "No data found.":
            LOGGER.debug("FETCH_DATA_ROWS: No data found for %s", context)
            return []
    ...
    async def _fetch_json_list(
        ...
        *,
        ...
        if isinstance(payload, dict):
            if payload.get("message") == "No data found." or not payload:
                LOGGER.debug("FETCH_JSON_LIST: No data found for %s", context)
                return []
